...
async function fetchGithubStars(githubUrl) {
  try {
    const apiUrl = githubUrl.replace(
      "https://github.com/",
...
async function fetchAndUploadPlugins() {
  try {
    ...
    const plugins = await Promise.all(
      notionData
        .filter((item) => item.properties["Approved?"].checkbox)
        .map(async (item) => {
          const npmUrl =
            item.properties["NPM page"]?.rich_text[0]?.plain_text || "";
          const githubUrl =
            item.properties["Github repository"]?.rich_text[0]?.plain_text ||
            "";
          ...
          return {
            ...
            docs:
              item.properties["Documentation"]?.rich_text[0]?.plain_text || "",
            tags:
              item.properties["Tags"]?.rich_text[0]?.plain_text.split(",") ||
              [],
